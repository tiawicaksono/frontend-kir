...
export default function AntrianUjiFilter({
  filters,
  onChange,
  onReload,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setJenisOptions(
          (res.data || res)
            .filter((i: any) => [1, 2, 7, 8].includes(Number(i.value)))
            .map((i: any) => ({
              label: i.label,
              value: i.value,
            })),
  ...
  return (
    <div className="flex justify-between mb-4 gap-2 flex-wrap">
      <Space wrap>
        ...
        {/* ========================= */}
        <Select
          placeholder="Status Datang"
          allowClear
          style={{ width: 180 }}
          disabled={loading}
          value={
            filters?.status_datang === undefined
              ? undefined
              : String(filters.status_datang)
          }
          onChange={(val) =>
            onChange({
              status_datang: val === undefined ? undefined : val === "true",
